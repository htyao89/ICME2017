...
addpath('./yael_v438/matlab/')	;
    ...
    [gallery_list,~] = textread('../person-/test.txt','%s %d');
    if exist('../../icme2017_bk/featureExtraction/test_feat.mat')
    	tmp_feat=load('../../icme2017_bk/featureExtraction/test_feat.mat');
	gallery_feat = tmp_feat.test_feat;
	gallery_feat = gallery_feat(1:length(gallery_list),:);  
    else
	gallery_feat = load('../../icme2017_bk/featureExtraction/reid_test_feat.txt');
	gallery_feat = gallery_feat(1:length(gallery_list),:); 
    end
    ...
    [query_list,~] = textread('../../../dataset/query.txt','%s %d');
    query_feat=importdata('../../icme2017_bk/featureExtraction/reid_query_feat.txt');
